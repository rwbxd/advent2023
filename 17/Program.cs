...
long part2(List<List<int>> grid) {
    ...
    PriorityQueue<NodeNode, int> pq = new();
    NodeNode nn;
    NodeNode zero = new NodeNode(nodeGrid[0][0], '?', 0, 0);
    nn = createNodeNode4(zero, nodeGrid[0][1], 'R');
    pq.Enqueue(nn, nn.Cost + dist(nn.N, goal));
    // cost[nn] = 0;
    nn = createNodeNode4(zero, nodeGrid[1][0], 'D');
    pq.Enqueue(nn, nn.Cost + dist(nn.N, goal));
    // cost[nn] = 0;
    ...
    while (pq.Count != 0) {
        ...
        if (bar(curNN, 'D') && foo(ci+1, cj, nextCost) && !(curNN.D != 'D' && !foo(ci+4, cj, nextCost))) {
            newNN = createNodeNode4(curNN, nodeGrid[ci+1][cj], 'D');
            Console.WriteLine(string.Format("Adding ({0}, {1}) 'D' with cost: {2}", newNN.N.I, newNN.N.J, newNN.Cost));
            pq.Enqueue(newNN, newNN.Cost + dist(newNN.N, goal));
        }

        if (bar(curNN, 'U') && foo(ci-1, cj, nextCost) && !(curNN.D != 'U' && !foo(ci-4, cj, nextCost))) {
            // cost[nodeGrid[ci-1][cj]] = nextCost;
            newNN = createNodeNode4(curNN, nodeGrid[ci-1][cj], 'U');
            Console.WriteLine(string.Format("Adding ({0}, {1}) 'U' with cost: {2}", newNN.N.I, newNN.N.J, newNN.Cost));
            pq.Enqueue(newNN, newNN.Cost + dist(newNN.N, goal));
        }

        if (bar(curNN, 'R') && foo(ci, cj+1, nextCost) && !(curNN.D != 'R' && !foo(ci, cj+4, nextCost))) {
            // cost[nodeGrid[ci][cj+1]] = nextCost;
            newNN = createNodeNode4(curNN, nodeGrid[ci][cj+1], 'R');
            Console.WriteLine(string.Format("Adding ({0}, {1}) 'R' with cost: {2}", newNN.N.I, newNN.N.J, newNN.Cost));
            pq.Enqueue(newNN, newNN.Cost + dist(newNN.N, goal));
        }

        if (bar(curNN, 'L') && foo(ci, cj-1, nextCost) && !(curNN.D != 'L' && !foo(ci, cj-4, nextCost))) {
            // cost[nodeGrid[ci][cj-1]] = nextCost;
            newNN = createNodeNode4(curNN, nodeGrid[ci][cj-1], 'L');
            Console.WriteLine(string.Format("Adding ({0}, {1}) 'L' with cost: {2}", newNN.N.I, newNN.N.J, newNN.Cost));
            pq.Enqueue(newNN, newNN.Cost + dist(newNN.N, goal));
        }
    }
...
NodeNode createNodeNode4(NodeNode cur, Node next, char dir) {
    if (cur.D == dir) {
        return new NodeNode(next, dir, cur.CD + 1, cur.Cost + grid[next.I][next.J]);
    }

    if (cur.N.I == next.I) {
        int newCost = grid[next.I][next.J];
        for (int j = 1; j < 4; j++) {
            newCost += grid[next.I][next.J + ((next.J - cur.N.J) * j)];
        }
        Console.WriteLine("new cost: " + newCost.ToString());
        return new NodeNode(new Node(next.I, next.J + ((next.J - cur.N.J) * 3)), dir, 4, cur.Cost + newCost);
    } else {
        int newCost = grid[next.I][next.J];
        for (int i = 1; i < 4; i++) {
            newCost += grid[next.I + ((next.I - cur.N.I) * i)][next.J];
        }
        Console.WriteLine("new cost: " + newCost.ToString());
        return new NodeNode(new Node(next.I + ((next.I - cur.N.I) * 3), next.J), dir, 4, cur.Cost + newCost);
    }
}
