...
void Part2() {
    ...
    int curLength = 1;
    int R = 0; int L = 0; int U = 0; int D = 0;
    HashSet<string> hashmarks = new HashSet<string>();
    hashmarks.Add("(0,0)");
    for (int i = 0; i < dirs.Count; i++) {
        switch (dirs[i]) {
            case 'R':
    ...
    for (int i = 0; i < length; i++) {
        Console.WriteLine("Row " + i.ToString() + "/" + length.ToString());
        inside = false;
        for (int j = 0; j < width; j++) {
            if (hashmarks.Contains(string.Format("({0},{1})", i, j))) {
                area++;
                if (!hashmarks.Contains(string.Format("({0},{1})", i, j-1)) && !hashmarks.Contains(string.Format("({0},{1})", i, j+1))) {
                    inside = !inside;
                } else {
                    if (!hashmarks.Contains(string.Format("({0},{1})", i-1, j))) {
                        inside = !inside;
                    }
                }
            } else {
